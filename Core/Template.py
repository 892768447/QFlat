...
from Core.Util import ObjectDict, exec_in, unicode_type, PY3

if PY3:
    from io import StringIO  # @UnusedImport
else:
    from cStringIO import StringIO  # @UnresolvedImport @Reimport
...
class DictLoader(BaseLoader):
    ...
    def __init__(self, dict, **kwargs):  # @ReservedAssignment
...
class _CodeWriter(object):
    ...
    def indent(self):
        class Indenter(object):
            def __enter__(_):  # @NoSelf
                self._indent += 1
                return self

            def __exit__(_, *args):# @NoSelf @UnusedVariable
...
def _format_code(code):
    ...
    format = "%%%dd  %%s\n" % len(repr(len(lines) + 1))  # @ReservedAssignment
...
def _parse(reader, template, in_block=None, in_loop=None):
    ...
    while True:
        ...
        operator, space, suffix = contents.partition(" ")  # @UnusedVariable
        suffix = suffix.strip()
